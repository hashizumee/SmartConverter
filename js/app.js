...
function initExcelConverter() {
    ...
    exc.dropzone.addEventListener('click', (e) => {
        if (e.target !== exc.fileInput) {
            exc.fileInput.click();
        }
    });
    ...
    // Mode/Type Selection change toggles UI fields
    if (exc.typeColumn) {
        exc.typeColumn.addEventListener('change', (e) => {
            if (e.target.value === 'all') {
                exc.rowNodeGroup.style.display = 'block';
            } else {
                exc.rowNodeGroup.style.display = 'none';
            }
        });
    }
    ...
    if (exc.sheetSelect) {
        exc.sheetSelect.addEventListener('change', () => {
            updateExcelTypeSelector();
        });
    }
...
function updateExcelTypeSelector() {
    const exc = elements.excel;
    if (!state.excel.workbook) return;
    
    const selectedSheet = exc.sheetSelect.value || state.excel.sheetNames[0];
    const sheet = state.excel.workbook.Sheets[selectedSheet];
    ...
    for (let C = range.s.c; C <= range.e.c; ++C) {
        const cell = sheet[window.XLSX.utils.encode_cell({c: C, r: range.s.r})];
        let hdr = "Kolom_" + window.XLSX.utils.encode_col(C); 
        if(cell && cell.v !== undefined) {
            hdr = window.XLSX.utils.format_cell(cell).trim();
        }
        headers.push({id: window.XLSX.utils.encode_col(C), name: hdr});
    }
    
    // Save Kolom A header as the keyHeader (Parameter)
    state.excel.keyHeader = headers[0] ? headers[0].name : '';
    
    let optionsHtml = `<option value="all">Semua Kolom (Konversi Standar)</option>`;
    
    // Remaining columns (Column B onwards) are the values (Types)
    for (let i = 1; i < headers.length; i++) {
        const h = headers[i];
        if (h.name) {
            optionsHtml += `<option value="${h.name}">${h.name} (Kolom ${h.id})</option>`;
        }
    }
    
    if (exc.typeColumn) {
        exc.typeColumn.innerHTML = optionsHtml;
        exc.typeColumn.value = 'all'; // Default to standard
    }
    
    if (exc.typeSelectGroup) {
        exc.typeSelectGroup.style.display = 'block';
    }
    if (exc.rowNodeGroup) {
        exc.rowNodeGroup.style.display = 'block';
    }
...
function performExcelToXmlConversion() {
    ...
    const selectedSheet = exc.sheetSelect.value || state.excel.sheetNames[0];
    const selectedType = exc.typeColumn ? exc.typeColumn.value : 'all';
    
    // Pull form options
    const options = {
        mode: selectedType === 'all' ? 'standard' : 'key_value',
        keyColumn: state.excel.keyHeader || '',
        valueColumn: selectedType,
...
function initXmlConverter() {
    ...
    // Trigger upload
    xm.dropzone.addEventListener('click', (e) => {
        if (e.target !== xm.fileInput) {
            xm.fileInput.click();
        }
    });
